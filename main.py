...
class DetThread(QThread):
    ...
    # send_pic = pyqtSignal(np.ndarray)
    ...
    def run(self,
        ...
        try:
            ...
            while True:
                ...
                # 暂停开关
                if self.is_continue:
                    path, img, img0, img1, self.vid_cap = next(dataset)
                    print(path, '$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$')
                    print(img.shape, '**********************************************')  # 送入网络之前的大小
                    print(img0.shape, '&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&')  # 原图大小
...
#
                    ...
                    self.send_raw.emit(img0)
                    #self.send_raw.emit(im0s if isinstance(im0s, np.ndarray) else im0s[0])#官方用


                    # self.send_pic.emit(im0s if isinstance(im0s, np.ndarray) else im0s[0])
...
class MainWindow(QMainWindow, Ui_mainWindow):
    def __init__(self, parent=None):
        ...
        # self.det_thread.send_pic.connect(self.show_image1)
    ...
    def open_file(self):
        ...
        if not os.path.exists(open_fold):
            open_fold = os.getcwd()
        self.name, _ = QFileDialog.getOpenFileName(self, '选取视频或图片', open_fold,
                                                   "Pic File(*.mp4 *.mkv *.avi *.flv "
                                                   "*.jpg *.png)")
        if self.name:
            self.det_thread.source = self.name
            # print(self.name)
            self.statistic_msg('加载文件：{}'.format(os.path.basename(self.name)))
            config['open_fold'] = os.path.dirname(self.name)
            config_json = json.dumps(config, ensure_ascii=False, indent=2)
            # if self.name.endswith(('.mp4')):
            # self.det_thread.send_pic.disconnect(self.show_image1)
            with open(config_file, 'w', encoding='utf-8') as f:
                f.write(config_json)
            # 切换文件后，上一次检测停止
            self.stop()
    ...
    def show_image1(self):
        ...
        self.src_img1 = self.name
        # print(self.src_img)
        src_img = cv2.imdecode(np.fromfile(self.src_img1, dtype=np.uint8), -1)
    ...
    def process_image(self):
        # print('po')
        self.src_img = cv2.imread(self.name)
        img = self.src_img.copy()
        for i in range(self.useListWidget.count()):
            name = self.useListWidget.item(i).text()
            img = self.useListWidget.item(i)(img)
            cv2.imencode('.jpg', img)[1].tofile(f'C:/Users/Lenovo/PycharmProjects/PyQt5-YOLOv5-master/final/{name}.jpg')
            # cv2.imwrite(,img)
        # print(999)
        return img
